...
async function main() {
  ...
  try {
    ...
    const headers = { Authorization: `Keydrop ${token}`, "X-Keydrop-Lease": lease };

    const claim = await request(options.endpoint, "/api/v1/drop/claim", { method: "POST", headers });
    if (claim.status !== 204) throw new Error("canary claim failed");
    const payload = await request(options.endpoint, "/api/v1/drop", { headers });
    ...
    const ack = await request(options.endpoint, "/api/v1/drop/ack", { method: "POST", headers });
    if (ack.status !== 204) throw new Error("canary acknowledgement failed");
    const retry = await request(options.endpoint, "/api/v1/drop/ack", { method: "POST", headers });
    if (retry.status !== 204) throw new Error("canary acknowledgement retry failed");
    const replay = await request(options.endpoint, "/api/v1/drop", { headers });
